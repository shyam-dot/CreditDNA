...
import { AlertCircle, Cpu, ArrowLeft, Mail, Lock, User, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else { await signUpWithEmail(name.trim(), email, password); }
      navigate('/dashboard', { replace: true });
    } catch (err: unknown) {
      ...
      } else { setError(raw || 'Authentication failed. Please try again.'); }
    } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-[#0B0F19] text-gray-100 flex flex-col items-center justify-center px-4 relative selection:bg-[#00D4FF]/30 selection:text-[#00D4FF]">
      ...
      <div className="absolute top-6 left-6">
        <Link to="/" className="inline-flex items-center gap-2 text-xs font-mono text-gray-400 hover:text-white transition-colors">
          <ArrowLeft size={14} /><span>BACK TO LANDING</span>
        </Link>
      </div>
      <div className="mb-8 flex items-center gap-3 relative z-10">
        ...
        <div className="flex flex-col">
          <span className="text-2xl font-space font-extrabold tracking-tight text-white">Credit<span className="text-[#00D4FF]">DNA</span></span>
      ...
      <div className="w-full max-w-md glass-card p-8 border border-white/10 relative z-10 shadow-2xl">
        <h1 className="text-2xl font-space font-bold text-white mb-1 text-center">Terminal Authentication</h1>
        <p className="text-xs font-mono text-gray-400 leading-relaxed text-center mb-6">// Access your Financial DNA &amp; Resilience Profile</p>
        <div className="flex rounded-xl overflow-hidden border border-white/10 mb-6">
          {(['login', 'signup'] as Tab[]).map((t) => (
            <button key={t} onClick={() => { setTab(t); setError(''); }}
              className={`flex-1 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-all duration-200 ${tab === t ? 'bg-[#00D4FF] text-[#0B0F19]' : 'bg-white/5 text-gray-400 hover:text-white'}`}>
              {t === 'login' ? 'Sign In' : 'Sign Up'}
            </button>
          ))}
        </div>
        <button type="button" onClick={handleGoogleSignIn} disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 hover:bg-gray-100 active:scale-[0.98] py-3 px-4 rounded-xl font-mono text-xs font-bold shadow-lg transition-all duration-200 mb-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-[10px] font-mono text-gray-500 uppercase">or with email</span>
          <div className="flex-1 h-px bg-white/10" />
        </div>
        <form onSubmit={handleEmailAuth} className="space-y-3">
          {tab === 'signup' && (
            <div className="relative">
              ...
              <input type="text" placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-3 text-xs font-mono text-white placeholder-gray-600 focus:outline-none focus:border-[#00D4FF]/60 transition-colors" />
            </div>
          )}
          <div className="relative">
          ...
          <div className="relative">
            ...
            <input type={showPassword ? 'text' : 'password'} placeholder="Password (min. 6 chars)" value={password} onChange={(e) => setPassword(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-10 py-3 text-xs font-mono text-white placeholder-gray-600 focus:outline-none focus:border-[#00D4FF]/60 transition-colors" />
          ...
          <button type="submit" disabled={loading} className="w-full btn-primary py-3 text-xs font-mono font-bold mt-1">
            {loading ? <span className="w-4 h-4 border-2 border-[#0B0F19] border-t-transparent rounded-full animate-spin" /> : tab === 'login' ? 'SIGN IN' : 'CREATE ACCOUNT'}
          </button>
        </form>
        <p className="mt-5 text-[11px] font-mono text-gray-500 text-center">
